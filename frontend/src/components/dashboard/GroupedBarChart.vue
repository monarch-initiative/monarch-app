<template>
  <BaseChart
    ...
    ref="baseChartRef"
  >
    <!-- The actual Grouped Bar chart will be rendered by ECharts in the BaseChart canvas -->
...
/** Process gene connection data into grouped bar format */
const groupedData = computed(() => {
  if (!queryResult.value?.data || queryResult.value.data.length === 0) {
    return { categories: [], series: [] };
  }

  // Process data to compare ortholog vs human-only information sources
  const categories = ['Genes with Ortholog Info', 'Genes with Human Info Only'];
  ...
  const genesWithOrthologInfo = processedData
    .filter(d => d.hasOrtholog && d.hasOrthologPhenotype)
    .reduce((sum, d) => sum + d.count, 0);
    
  const genesWithHumanOnlyInfo = processedData
    .filter(d => d.hasHumanPhenotype && !d.hasOrthologPhenotype)
    .reduce((sum, d) => sum + d.count, 0);

  const genesWithBothInfo = processedData
    .filter(d => d.hasHumanPhenotype && d.hasOrthologPhenotype)
    .reduce((sum, d) => sum + d.count, 0);

  const genesWithOrthologOnlyInfo = processedData
    .filter(d => !d.hasHumanPhenotype && d.hasOrthologPhenotype)
    .reduce((sum, d) => sum + d.count, 0);
  ...
  return { categories, series, processedData };
});
...
const updateChart = () => {
  if (baseChartRef.value?.chart && groupedData.value.categories.length > 0) {
