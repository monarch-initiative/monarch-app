...
const barData = computed(() => {
  ...
  queryResult.value.data.forEach((row: any) => {
    ...
    const category = row.category || row.name || row.subject_category || row.object_category || row.label || 'Unknown';
    // Try different possible column names for count
    const count = Number(row.count || row.value || row.total || row.frequency || 0);

    const cleanCategory = typeof category === 'string' ? 
      category.replace('biolink:', '') : String(category);
