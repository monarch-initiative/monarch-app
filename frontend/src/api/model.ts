...
/**
 * The directionality of an association as it relates to a specified entity,
 * with edges being categorized as incoming or outgoing
 */
export enum AssociationDirectionEnum {
  /**
   * An association for which a specified entity is the object or part of the
   * object closure
   */
  incoming = "incoming",
  /**
   * An association for which a specified entity is the subject or part of the
   * subject closure
   */
  outgoing = "outgoing",
}
...
export interface TermPairwiseSimilarity extends PairwiseSimilarity {
  ...
  /**
   * the most recent common ancestor of the two compared entities. If there are
   * multiple MRCAs then the most informative one is selected
   */
  ancestor_id?: string;
  ...
  /**
   * The number of concepts in the intersection divided by the number in the
   * union
   */
  jaccard_similarity?: number;
  /**
   * the dot product of two node embeddings divided by the product of their
   * lengths
   */
...
export interface Association {
   ...
   * source uses before transformation to match the biolink-model
   * specification.
   */
   ...
   * reasoning or analysis methods used to generate the statement, or the scope
  ...
  has_total?: number;
  /**
   * percentage, which may be calculated from has_count and has_total, as 100
   *
   * - quotient or provided directly, rounded to the integer level
   */
  ...
  /**
   * A context qualifier representing a disease or condition in which a
  ...
  /**
   * Field containing disease_context_qualifier id and the ids of all of it's
   * ancestors
   */
  disease_context_qualifier_closure?: string[];
  /**
   * Field containing disease_context_qualifier name and the names of all of
   * it's ancestors
   */
  disease_context_qualifier_closure_label?: string[];
  /**
   * A context qualifier representing a species in which a relationship
  ...
  /**
   * A qualifier that composes with a core subject/object concept to define a
   * more specific version of the subject concept, specifically using an
   * ontology term that is not a subclass or descendant of the core concept and
   * in the vast majority of cases, is of a different ontological namespace than
   * the category or namespace of the subject identifier.
   */
  ...
  /**
   * A closure of the subject_specialization_qualifier, including the
  ...
  /**
   * A qualifier that composes with a core subject/object concept to define a
   * more specific version of the object concept, specifically using an ontology
   * term that is not a subclass or descendant of the core concept and in the
   * vast majority of cases, is of a different ontological namespace than the
   * category or namespace of the object identifier.
   */
  ...
  /**
   * A closure of the object_specialization_qualifier, including the
   * object_specialization_qualifier itself and all of its ancestors
   */
...
export interface CategoryGroupedAssociationResults extends Results {
  /**
   * The category of the counterpart entity in a given association, eg. the
...
export interface Entity {
  ...
  full_name?: string;
  /**
   * A boolean flag indicating that an entity is no longer considered current or
   * valid.
   */
  deprecated?: boolean;
  description?: string;
  ...
  namespace?: string;
  /**
   * A list of phenotype identifiers that are known to be associated with this
   * entity
   */
  ...
  /**
   * A list of phenotype labels that are known to be associated with this entity
   * expanded to include all ancestors
   */
  has_phenotype_closure_label?: string[];
  /**
   * A count of the number of phenotypes that are known to be associated with
   * this entity
   */
...
/** Optional highlighting fields with HTML markup for search results */
...
/**
 * An association with directionality and optional highlighted text (e.g., <em>)
 * from Solr or search results
 */
