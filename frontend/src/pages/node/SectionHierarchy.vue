  ...
  <div class="toc-hier" role="group" aria-label="Hierarchy preview">
    ...
    <div class="parents">
      <div v-for="p in parents" :key="p.id" class="parent-row">
        <RouterLink v-tooltip="labelOf(p)" :to="`/${p.id}`" class="row-text">
          {{ labelOf(p) }}
        </RouterLink>
      </div>
    </div>
    ...
    <div class="current-row">
      <strong v-tooltip="labelOf(node)" class="row-text">{{
        labelOf(node)
      }}</strong>
    </div>
    ...
    <div class="children">
      <div v-for="c in shownChildren" :key="c.id" class="child-row">
        <RouterLink v-tooltip="labelOf(c)" :to="`/${c.id}`" class="row-text">
          {{ labelOf(c) }}
        </RouterLink>
      </div>
