...
def _search(
    ...
    metric: SemsimMetric = Query(SemsimMetric.ANCESTOR_INFORMATION_CONTENT, title="Similarity metric to use"),
    directionality: SemsimDirectionality = Query(SemsimDirectionality.BIDIRECTIONAL, title="Directionality of the search"),
    ...
    results = semsimian().search(termset=terms, prefix=parse_similarity_prefix(group), metric=metric, directionality=directionality, limit=limit)
    return results
